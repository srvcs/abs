use srvcs_abs::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-abs listening");
    let deps = Deps {
        isnegative_url: cfg.isnegative_url.clone(),
        negate_url: cfg.negate_url.clone(),
    };
